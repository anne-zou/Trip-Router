...
public class ItineraryLegIconView extends View {
    ...
    private static final String TAG = ItineraryLegIconView.class.getName();
    ...
    private final int ROUTE_TEXT_SIZE = 11;

    private final int ROUTE_TEXT_PADDING = 4;

    private final int SPACE_BETWEEN_ICONS = 5;

    private final float ROUNDED_RECT_RADIUS = 4;

    private final int DURATION_TEXT_SIZE = 10;
    ...
    private int paddingBetweenModeIconAndDurationText = 3;
    ...
    public ItineraryLegIconView(Context context, int paddingBetweenModeIconAndDurationText) { // Set duration padding to 0 for bike
        this(context);
        this.paddingBetweenModeIconAndDurationText
    ...
    public ItineraryLegIconView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (isShowRoute) {
            ...
            mLegDurationPaint.setTextSize(PixelUtil.pxFromDp(mContext, DURATION_TEXT_SIZE));
        }
    ...
    private void updateContentBounds() {
        ...
        if (!isShowRoute) {
            ...
            if (mIcon != null) {
                ...
                float modeIconAnchorX = rightBound
                        + PixelUtil.pxFromDp(mContext, paddingBetweenModeIconAndDurationText)
            ...
            if (mIcon != null) {
                ...
                if (PixelUtil.pxFromDp(mContext, ICON_WIDTH) != -1) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isShowRoute) {
            canvas.drawRoundRect(mRouteIconRect,
                    PixelUtil.pxFromDp(mContext, (int) ROUNDED_RECT_RADIUS),
                    PixelUtil.pxFromDp(mContext, (int) ROUNDED_RECT_RADIUS),
                    mRouteBackgroundPaint);
    ...
    public int getModeIconWidth() {
        ...
        else return PixelUtil.pxFromDp(mContext, ICON_WIDTH);
    }
